...
| `maxStreamingConnections`          | `MAX_STREAMING_CONNECTIONS`           |  Number  | none    | Maximum number of concurrent server-side streaming connections (`/all`, `/flags`) Relay will admit. Connections over this limit receive an immediate `503` with `Retry-After` instead of being accepted and queued behind data-source replay, which can exhaust memory under a connection storm against large environments. Unset means unlimited (prior behavior).                                                                                                            |
