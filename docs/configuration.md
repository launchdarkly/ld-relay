...
| `maxClientWriteTime`               | `MAX_CLIENT_WRITE_TIME`               | Duration | none    | Maximum amount of time that Relay will allow a single write to a streaming connection to take. A client that has stopped reading is disconnected once a write exceeds this, instead of holding the connection and its buffered data open indefinitely. _(11)_                                                                                                                                                                                                                      |
...
_(11)_ The optional `maxClientWriteTime` setting bounds each write to a stream client: one event, or the initial `put` with all of the environment's flag data. The time Relay spends serializing the data does not count. The `put` and the flush that follows it are bounded separately, so a client can take up to twice the value to receive it. Set it above the time your slowest legitimate client needs to receive the largest `put`, since a client that is disconnected reconnects and downloads the whole payload again; too small a value adds load instead of shedding it. It does not bound the connection, so a client that reads slowly but steadily can still hold one open unless `maxClientConnectionTime` is also set. Each stream disconnected by this setting logs a warning. With TLS enabled it also sets the HTTP/2 connection write timeout, so a client that stops reading a whole connection is disconnected too, after up to 5 more seconds spent closing TLS; a stream that was mid-write logs the same warning, but idle streams and other responses on that connection do not. That timeout applies to every response on the connection, not just streams, and is not set when Relay is embedded as a library in your own `http.Server`. With `enableCompression`, a stalled flush is detected at the next write, up to one `heartbeatInterval` later. The value must be `0` (disabled) or at least `1s`.
