...
| `maxStreamingConnections`          | `MAX_STREAMING_CONNECTIONS`           |  Number  | auto-detected | Maximum number of concurrent server-side streaming connections (`/all`, `/flags`) Relay will admit. Connections over this limit receive an immediate `503` with `Retry-After`. If unset, Relay derives a default from detected available memory (the cgroup limit if present, otherwise total system memory); set explicitly to override, or to a very large number to effectively disable.                                                                                   |
