...
| `cacheEncryptionKey`     | `AUTO_CONFIG_CACHE_ENCRYPTION_KEY`    | String  | _(auto)_ | Key used to encrypt the cached AutoConfig in the store. Can be any string; it is derived to a 256-bit key via SHA-256. If omitted, the AutoConfig key is used. _(7)_                                                                 |
