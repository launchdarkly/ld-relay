...
_(11)_ The optional `maxClientWriteTime` setting bounds each write to a stream client: one event, or the initial `put` with all of the environment's flag data. Set it above the time your slowest legitimate client needs to receive the largest `put`, since a client that is disconnected reconnects and downloads the whole payload again; too small a value adds load instead of shedding it. It does not bound the connection, so a client that reads slowly but steadily can still hold one open unless `maxClientConnectionTime` is also set. Each disconnect logs a warning. The value must be at least `1s`.
