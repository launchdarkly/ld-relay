...
| `initFromStoreFirst`     | `AUTO_CONFIG_INIT_FROM_STORE_FIRST`   | Boolean | `false` | If true, Relay loads AutoConfig from the persistent store (Redis/Valkey or DynamoDB) on startup before connecting to LaunchDarkly, so it can serve traffic when the AutoConfig service is offline. Requires Redis or DynamoDB and `cacheKey`. _(7)_ |
| `cacheKey`               | `AUTO_CONFIG_CACHE_KEY`                | String  |         | Key used to store and retrieve the cached AutoConfig snapshot in the persistent store. Required when `initFromStoreFirst` is true. Multiple Relay instances can share the same store by using different cache keys. _(7)_            |
| `cacheEncryptionKey`     | `AUTO_CONFIG_CACHE_ENCRYPTION_KEY`    | String  | _(auto)_ | 32-byte key (or base64-encoded 32 bytes) used to encrypt the cached AutoConfig in the store. If omitted, the AutoConfig key is used (SHA-256 derived to 32 bytes). _(7)_                                                                 |
...
_(7)_ When `initFromStoreFirst` is true, Relay reads the cached AutoConfig from the store at startup (if present), applies it so the proxy can serve traffic immediately, then connects to the LaunchDarkly AutoConfig stream. Whenever the stream sends a full put, Relay writes the new snapshot to the store (encrypted under `cacheKey`). If `cacheEncryptionKey` is set, it must be exactly 32 bytes or a base64 string that decodes to 32 bytes; otherwise the AutoConfig key is used (SHA-256 derived to 32 bytes). The cache uses the same Redis or DynamoDB connection as the rest of Relay.
