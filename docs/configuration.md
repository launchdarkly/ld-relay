...
| `environmentStatusMetrics` | `OTEL_ENVIRONMENT_STATUS_METRICS` | Boolean | `false` | If true, also exports the status of each environment as its own series, naming the environment. Off by default because it adds roughly 18 series per environment served, whether or not that environment receives traffic; the relay-level status metrics are exported either way. To learn more, read [Metrics](./metrics.md). |
