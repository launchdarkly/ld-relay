...
| `cacheKey`               | `AUTO_CONFIG_CACHE_KEY`                | String  |         | If set, enables AutoConfig caching. Relay loads cached AutoConfig from the persistent store (Redis/Valkey or DynamoDB) on startup before connecting to LaunchDarkly, and persists updates as they arrive. Requires Redis or DynamoDB. Multiple Relay instances can share the same store by using different cache keys. _(7)_ |
...
_(7)_ When `cacheKey` is set, Relay reads the cached AutoConfig from the store at startup (if present), applies it so the proxy can serve traffic immediately, then connects to the LaunchDarkly AutoConfig stream. As the stream sends updates, Relay writes them to the store. If `cacheEncryptionKey` is set, it can be any string and is derived to a 256-bit AES key via SHA-256; if omitted, the AutoConfig key is used. The cache uses the same Redis or DynamoDB connection as the rest of Relay.
