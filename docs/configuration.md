...
Two properties of this scope are deliberate and worth knowing when you size the budget: the `evalx` endpoints are the initialization path for client-side and mobile SDKs, so that path is not protected by this budget; and the budget is one pool shared by all environments and both protocol generations, reachable by any credential a gated endpoint accepts, including public client-side IDs. Isolation between principals is planned as a follow-up.
