...
> **Breaking change in v9+**: The Relay Proxy now uses the OpenTelemetry SDK as the single export pipeline for all backends. The deprecated OpenCensus exporter modules (`DataDog/opencensus-go-exporter-datadog`, `launchdarkly/opencensus-go-exporter-stackdriver`, and `contrib.go.opencensus.io/exporter/prometheus`) have been removed. This affects how each backend is wired up:
>
