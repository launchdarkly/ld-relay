...
- The `dataStoreStatus` properties are, for the most part, only relevant if you are using [persistent storage](./persistent-storage.md).
    ...
    - `dbServer`, if present, is the configured database URL or hostname. Because this endpoint is unauthenticated, any credentials in the URL are redacted: the userinfo, query, and fragment components are each replaced with `xxxxx`, and a URL that cannot be parsed is replaced entirely.
