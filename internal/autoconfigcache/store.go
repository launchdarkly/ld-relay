...
func NewStore(c config.Config, logger *slog.Logger) (Store, error) {
	...
	if c.Redis.URL.IsDefined() {
		return newRedisStore(c.Redis, cacheKey, encKey, logger)
	}
	return newDynamoDBStore(c.DynamoDB, cacheKey, encKey, logger)
}
