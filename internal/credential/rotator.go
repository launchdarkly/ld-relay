...
func (r *Rotator) expireMobileKey(mobileKey config.MobileKey) {
	r.logger.Info("deprecated mobile key has expired and is no longer valid for authentication", "key", mobileKey.Masked())
...
func reconcileAcceptedKeys[K reconcilableKey](
	...
	for key, want := range desired {
		if _, ok := accepted[key]; !ok {
			*additions = append(*additions, key)
			logger.Info("credential is now accepted", "kind", kind, "key", key.Masked())
		}
		accepted[key] = want
	}
	...
	for key := range accepted {
		...
		*expirations = append(*expirations, key)
		logger.Info("credential is no longer accepted and has been revoked", "kind", kind, "key", key.Masked())
	}
}
...
func (r *Rotator) reconcileSDKKeys(set AcceptedSet, now time.Time) {
	...
	reconcileAcceptedKeys(desired, r.acceptedSDKKeys, &r.additions, &r.expirations, r.logger, "SDK key")
	...
	// that key, and the stream handlers still authenticate it. CommitAnchor drops the entry when the
...
func (r *Rotator) reconcileMobileKeys(set AcceptedSet, now time.Time) {
	...
	reconcileAcceptedKeys(desired, r.acceptedMobileKeys, &r.additions, &r.expirations, r.logger, "mobile key")
