...
// TestProofExtendedProfilePersistsAcrossTheFirstConnectionLoop is an attempted disproof of
// "the extended profile actually slows retries": it checks whether the activation survives
// repeated passes through eventsource's CanRetryFirstConnection(-1) loop, and whether the
// per-profile counter really doubles from the extended base. It does, so the vector fails.
func TestProofExtendedProfilePersistsAcrossTheFirstConnectionLoop(t *testing.T) {
