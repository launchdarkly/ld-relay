...
func newRejectingStreamManager(t *testing.T, extendedDelay time.Duration) (
	sm *StreamManager, mockLog *logtest.MockHandler, requests func() int, closeServer func(),
) {
	t.Helper()
	var mu sync.Mutex
	count := 0
...
func TestNoRequestIsMadeAfterClose(t *testing.T) {
	sm, _, requests, closeServer := newRejectingStreamManager(t, 200*time.Millisecond)
	defer closeServer()
