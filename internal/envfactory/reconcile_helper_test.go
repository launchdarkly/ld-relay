...
// TestDuplicateValueMarkedViewScopedIsRejected covers a validation gap found in the August 2026
// review of PR #817, fixed as part of this port.
//
// The builder de-duplicates by credential value and keeps the first metadata recorded for a value, so
// filtering view-scoped entries one at a time is not enough: a payload that lists one value twice,
// once plain and once marked, admitted the value through the plain entry while appending the marked
// entry's name to the rejected list. Relay both accepted the credential and logged that it had
// rejected it, which is the worst outcome for anyone auditing the logs.
//
// Entry order does not matter, because the rule is now a property of the value rather than of an
// entry.
func TestDuplicateValueMarkedViewScopedIsRejected(t *testing.T) {
	t.Run("sdk keys", func(t *testing.T) {
		orders := []struct {
			name    string
			entries []ConcurrentKeyRep
		}{
			{
				name: "plain entry first",
				entries: []ConcurrentKeyRep{
					{Key: "default-sdk", Value: "sdk-default"},
					{Key: "shared-plain", Value: "sdk-shared", HasViews: false},
					{Key: "shared-viewed", Value: "sdk-shared", HasViews: true},
				},
			},
			{
				name: "marked entry first",
				entries: []ConcurrentKeyRep{
					{Key: "default-sdk", Value: "sdk-default"},
					{Key: "shared-viewed", Value: "sdk-shared", HasViews: true},
					{Key: "shared-plain", Value: "sdk-shared", HasViews: false},
		...
		for _, order := range orders {
			t.Run(order.name, func(t *testing.T) {
				rep := EnvironmentRep{
					EnvID:      "env-1",
					SDKKey:     SDKKeyRep{Value: "sdk-default"},
					MobKey:     "mob-default",
					SDKKeys:    order.entries,
					MobileKeys: []ConcurrentKeyRep{{Key: "default-mob", Value: "mob-default"}},
				}

				set, rejected, err := BuildAcceptedSet(rep.ToParams())
				require.NoError(t, err)
				assert.Contains(t, rejected, "shared-viewed")

				// The shared value is absent from the set entirely: no entry admits it.
				expected := mustBuild(t, credential.NewAcceptedSetBuilder().
					WithEnvironmentID("env-1").
					WithAnchor(credential.SDKKeyParams{Value: "sdk-default", Key: util.PtrOrNil("default-sdk")}).
					WithPrimaryMobileKey(credential.MobileKeyParams{Value: "mob-default", Key: util.PtrOrNil("default-mob")}))
				assert.Equal(t, expected, set)
			})
		}
	})
	...
	t.Run("mobile keys", func(t *testing.T) {
		rep := EnvironmentRep{
			...
			SDKKey:  SDKKeyRep{Value: "sdk-default"},
			MobKey:  "mob-default",
			SDKKeys: []ConcurrentKeyRep{{Key: "default-sdk", Value: "sdk-default"}},
			MobileKeys: []ConcurrentKeyRep{
				{Key: "default-mob", Value: "mob-default"},
				{Key: "shared-plain", Value: "mob-shared", HasViews: false},
				{Key: "shared-viewed", Value: "mob-shared", HasViews: true},
			},
		}
	...
	t.Run("a view marker on the anchor own entry is still disregarded", func(t *testing.T) {
		rep := EnvironmentRep{
			EnvID:  "env-1",
			SDKKey: SDKKeyRep{Value: "sdk-default"},
			SDKKeys: []ConcurrentKeyRep{
				{Key: "default-sdk", Value: "sdk-default", HasViews: true},
			},
		}
		...
		// Dropping the designated key would take the whole environment down.
		expected := mustBuild(t, credential.NewAcceptedSetBuilder().
			WithEnvironmentID("env-1").
			WithAnchor(credential.SDKKeyParams{Value: "sdk-default", Key: util.PtrOrNil("default-sdk")}))
		assert.Equal(t, expected, set)
