...
// The purpose of this test is to find a combination of environments/projects that violates either of two properties:
...
func TestProjectRouter_VerifySetProperty(t *testing.T) {
	...
	checkProperty := func(t *testing.T, nEnvironments, nProjects int) bool {
		logger, _ := logtest.NewMockLogger()
	...
	t.Run("exhaustive tests", func(t *testing.T) {
		...
		for nEnvironments := 0; nEnvironments < maxEnvironments; nEnvironments++ {
			...
			for nProjects := 1; nProjects < maxProjects; nProjects++ {
				if !checkProperty(t, nEnvironments, nProjects) {
					t.Fatalf("failed for (%v) projects, (%v) environments", nProjects, nEnvironments)
				}
			}
		}
	})
	...
	t.Run("spot checks", func(t *testing.T) {
		...
		for _, scenario := range scenarios {
			t.Run(fmt.Sprintf("%v environments %v projects", scenario.environments, scenario.projects), func(t *testing.T) {
				if !checkProperty(t, scenario.environments, scenario.projects) {
					t.Fail()
				}
			})
		}
