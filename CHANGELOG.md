...
* The metrics and tracing pipeline has been rebuilt on the OpenTelemetry SDK. The deprecated OpenCensus exporter modules (`DataDog/opencensus-go-exporter-datadog`, `launchdarkly/opencensus-go-exporter-stackdriver`, `contrib.go.opencensus.io/exporter/prometheus`) have been removed. See `docs/metrics.md` for the per-backend migration notes:
  * Datadog now requires a local Datadog Agent v7.43+ with OTLP receiver enabled; `DATADOG_TRACE_ADDR` is the OTLP gRPC endpoint, `DATADOG_STATS_ADDR` is deprecated and ignored.
  * Stackdriver uses Google's native OpenTelemetry exporters. Dashboards using `${prefix}/${metric}` metric types are unchanged.
  * Prometheus continues to serve on `/metrics` with the same port; the exposition format is now generated by the OpenTelemetry Prometheus exporter.
